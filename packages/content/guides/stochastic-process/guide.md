...
$X(\omega),\,Y(\omega)$をベクトルとみなしてみる。

$$
\begin{split}
R_{xx}&=\frac{1}{N_{\Omega}}\sum_{\omega \in \Omega}X(\omega)^2 \\
R_{yy}&=\frac{1}{N_{\Omega}}\sum_{\omega \in \Omega}Y(\omega)^2 \\
R_{xx}&=\frac{1}{N_{\Omega}}\sum_{\omega \in \Omega}X(\omega)Y(\omega)
\end{split}
$$

なので、$1/N_{\omega}$の差を無視すれば、$\sigma_x=\sqrt{R_{xx}},\,\sigma_y=\sqrt{R_{yy}}$はベクトルの長さを表し、$R_{xy}$はベクトルの内積を表すとみなせる。

したがって、内積の式より

$$
R_{xy}=\sigma_x\sigma_y\cos{\theta_{xy}}
$$

とかける。ここで $\cos{\theta_{xy}} \equiv \rho_{xy}$ は相関係数である。
$\rho_{xy}=0$ はベクトルの**直交**を表す。

$N$ 次元の場合も同様に考えられ、相関行列

$$
\mathbf{R}=[R_{mn}]
$$

はベクトルの内積を要素とする行列である。このような行列を**Gram行列**とよぶ。

$N$ 個のベクトル $(X_1,\cdots,X_N)$ が線型独立なら、それらのベクトルが張る斜方体の体積 $V_N$ の2乗に等しい。

$$
\mathrm{det}R=|R_{mn}|=V_N^2
$$

ベクトルが線形従属の場合、 $|R_{mn}|=0$ となり、斜方体が潰れて次元が低下することを示す。
すなわち、行列 $R_{mn}$ の階数は、線型独立な確率変数の数に等しい。

**【例7.1】 対角な相関行列**

互いに無相関な $N$ 次元確率変数 $\mathbf{Z}=(Z_1,\cdots,Z_N)^T$ は、幾何学的には、長さ $\sigma_n$ の互いに直行する $N$ 個のベクトルを表す。

$$
\begin{split}
\mathbf{R}&=[\sigma_n^2\delta_{mn}] \\
\mathrm{det}\mathbf{R}&=\prod_{n=1}^{N}\sigma_n^2
\end{split}
$$

と書ける。これより、そのベクトルの張る直方体の体積は $V_N=\prod_{n=1}^{N}\sigma_n$ となり、直感に一致する。

特に、分散を1に規格化した場合、

$$
\langle Y_mY_n \rangle=\delta_{mn}, \qquad \mathbf{R}=\mathbf{I}
$$

と表せる。つまり、 $Y_n$ はベクトルの基底になる。

$Y_n$ は**白色雑音**と呼ぶ確率変数の原型である。

#### 相関行列の変換

$N$ 次元確率変数 $X^0 \equiv (X_1^0,\cdots,X_N^0)^T$ が、線形変換により新しい $N$ 次元確率変数

$$
X_m=\sum_{n=1}^{N}a_{mn}X_{n}^{0}
$$
...
$$
R_{mn}=\sum_{j=1}^{N}\sum_{k=1}^{N}a_{mj}a_{nk}R_{jk}^0
$$

と変換される。これは相関行列が**２階テンソル**として変換されることを示す。

#### 直交変換

線形代数で学習するように、直交行列を用いて変換することもできる。

#### 相関行列の対角化と固有値問題

相関行列 $R$ をもつ $N$ 次元確率変数 $X$ が与えられたとき、それを、ある無相関な $N$ 次元確率変数 $Z$ への直交変換として表すことを考える。
これは、

$$
\mathbf{\sigma}^2=U^TRT
$$

と書け、直交行列 $U$ によって $R$ を対角行列 $\sigma^2$ に変換することを意味する。
つまり、行列 $R$ の**対角化**を表す。

このとき、 $\sigma_{\alpha}^2$ を**固有値**、$u_{\alpha}$ を固有ベクトルと呼ぶ。

このような直交展開を**スペクトル表現**といい、固有値 $\sigma_{\alpha}^2$ を**スペクトル**ともよぶ。

#### 確率変数の収束

確率変数をベクトルとみる考え方では、２つのベクトル $X,\,Y$ の間の距離の二乗は $\lVert X - Y \rVert^2 \equiv \langle (X-Y)^2 \rangle$ で与えられる。したがって、距離が0ならば、２つのベクトルは同一とみなす。つまり、

$$
\langle (X-Y)^2 \rangle =0 \quad \Rightarrow \quad X=Y
$$

次に、確率変数の列、 $X_1,\cdots,X_n$ があって、確率変数 $X$ との距離が0に近づく時、 $X_n$ は $X$ に**収束する**という。すなわち、

$$
\lim_{n \to \infty}\langle |X-X_n|^2 \rangle=0 \quad \Rightarrow \quad \lim_{n \to \infty}X_n = X 
$$

これを確率変数の（２乗）平均収束とよぶこともある。